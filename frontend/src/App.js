...
const EditTestModal = ({ test, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${API}/tests/${test.id}`, formData);
...
const EditClinicModal = ({ clinic, onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    name: clinic?.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API}/clinics/${clinic.id}`, updateData);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 max-h-96 overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">Edit Clinic</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <XCircle className="h-6 w-6" />
          </button>
        </div>
