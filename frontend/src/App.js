...
const CommunicationProviderForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess();
      alert(`${formData.role === 'clinic' ? 'Clinic/Hospital' : 'Lab Technician'} communication account created successfully!\n\nLogin Credentials:\nEmail: ${formData.email}\nPassword: ${formData.password}\n\nThe provider can now access their communication portal.`);
    } catch (error) {
      console.error('Error creating communication account:', error);
      alert('Error creating communication account. Please check if email already exists.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4 max-h-96 overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">Create Provider Communication Account</h2>
          <button onClick={() => setShow(false)} className="text-gray-500">
            <XCircle className="h-6 w-6" />
          </button>
        </div>
